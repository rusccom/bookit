...
export function OwnerCourtCard({ unit }: { unit: OwnerUnit }) {
  ...
  return <article className={styles.courtCard}>
    ...
    <div className={styles.tags}><span>{formatCourtKind(unit.kind)}</span><span>{formatCourtSurface(unit.surface)}</span><span>Слот: {formatSlotMinutes(unit.slotMinutes)}</span><strong>{formatPrice(unit.pricePerHour)}</strong></div>
