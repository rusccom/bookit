...
export function CourtIdentityFields({ unit }: { unit?: OwnerUnit }) {
  return <div className={styles.fieldGrid}>
    ...
    <label><span>Вид спорта</span><select defaultValue={unit?.kind || "tennis_court"} name="kind">{COURT_KIND_OPTIONS.map(renderOption)}</select></label>
    <label><span>Покрытие</span><select defaultValue={unit?.surface || "hard"} name="surface">{COURT_SURFACE_OPTIONS.map(renderOption)}</select></label>
  </div>;
}

function renderOption(item: { label: string; value: string }) {
