...
export function UpcomingBookingCard({ booking }: Props) {
  if (!booking) return <div className={shared.emptyCard}>
        <p>У вас пока нет предстоящих бронирований</p>
        <Link className="primary-link" href="/dashboard/customer/search">Забронировать корт</Link>
      </div>;
  return <div className={s.upcomingCard}>
      <div className={s.upcomingMeta}>
        <span className={s.upcomingDate}>{booking.dateLabel}, {booking.startTime} &ndash; {booking.endTime}</span>
        <span className={s.upcomingVenue}>{booking.venueTitle} / {booking.unitTitle}</span>
        <span className={s.upcomingAddress}>{booking.city}, {booking.address}</span>
