...
export function CourtAvailabilityCard(props: Props) {
  ...
  return <article className={styles.courtCard}>
    <header className={styles.courtHeader}><div><p>{item.venueTitle}</p><h2>{item.unitTitle}</h2><span>{item.city}, {item.address}</span></div><strong>{formatHourlyPrice(item.pricePerHour, "Цена по запросу")}</strong></header>
